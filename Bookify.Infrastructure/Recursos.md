...
Las pruebas de handler pueden sustituir `IUserContext` para representar al usuario local y deben comprobar reserva propia (exito), ajena (fallo `BookingErrors.NotFound`) e inexistente (el mismo fallo). Conviene usar GUID distintos para la identidad externa y el usuario local, para detectar confusiones entre ambos.

Las pruebas HTTP deben recorrer autenticacion, transformacion y controlador. Deben comprobar los resultados de los ejemplos, un identificador de ruta invalido, claims locales ausentes y usuario sin registro local. Al corregir la proteccion del endpoint, las solicitudes sin JWT o con JWT invalido deben dar `401` independientemente de si la reserva existe. Invocar directamente el controlador no verifica esa proteccion.
